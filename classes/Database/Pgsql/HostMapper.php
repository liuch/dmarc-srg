    ...
    public function statistics(array &$data, int $user_id): array
    {
        ...
        try {
            $st = $db->prepare(
                ...
                . ' INNER JOIN ' . $this->connector->tablePrefix('reports') . ' AS rp ON rr.report_id = rp.id' . $user_domains1
                . ' WHERE rr.ip = ?' . $user_domains2 . ' GROUP BY report_id) as t'
            );
            $st->bindValue(1, $data['ip'], \PDO::PARAM_STR);
            $st->execute();
            ...
            $st = $db->prepare(
                ...
                . ' INNER JOIN ' . $this->connector->tablePrefix('reports') . ' AS rp ON rr.report_id = rp.id' . $user_domains1
                . ' WHERE rr.ip = ?' . $user_domains2 . ' GROUP BY report_id, rp.id ORDER BY begin_time DESC LIMIT 2'
            );
            $st->bindValue(1, $data['ip'], \PDO::PARAM_STR);
            $st->execute();
            $last_report = [];
