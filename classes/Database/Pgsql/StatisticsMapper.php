...
class StatisticsMapper implements StatisticsMapperInterface
{
    ...
    public function summary($domain, array &$range, array &$filter, int $user_id): array
    {
        ...
        try {
            ...
            $st = $db->prepare(
                'SELECT SUM(rcount), SUM( CASE WHEN dkim_align = 2 AND spf_align = 2 THEN rcount ELSE 0 END),'
                . ' SUM(CASE WHEN dkim_align = 2 AND spf_align <> 2 THEN  rcount ELSE 0 END),'
                . ' SUM(CASE WHEN dkim_align <> 2 AND spf_align = 2 THEN rcount ELSE 0 END),'
                . ' SUM(CASE WHEN disposition = 0 THEN rcount ELSE 0 END),'
                . ' SUM(CASE WHEN disposition = 1 THEN rcount ELSE 0 END)'
    ...
    public function ips($domain, array &$range, array &$filter, int $user_id): array
    {
        try {
            ...
            $st = $this->connector->dbh()->prepare(
                ...
                . ' SUM(CASE WHEN dkim_align = 2 AND spf_align = 2 THEN rcount ELSE 0 END) AS dkim_spf_aligned,'
                . ' SUM(CASE WHEN dkim_align = 2 AND spf_align <> 2 THEN rcount ELSE 0 END) AS dkim_aligned,'
                . ' SUM(CASE WHEN dkim_align <> 2 AND spf_align = 2 THEN rcount ELSE 0 END) AS spf_aligned,'
                . ' SUM(CASE WHEN disposition = 0 THEN rcount ELSE 0 END),'
                . ' SUM(CASE WHEN disposition = 1 THEN rcount ELSE 0 END)'
                . ' FROM ' . $this->connector->tablePrefix('rptrecords') . ' AS rr'
                . ' INNER JOIN ' . $this->connector->tablePrefix('reports')
    ...
    public function organizations($domain, array &$range, array &$filter, int $user_id): array
    {
        try {
            ...
            $st = $this->connector->dbh()->prepare(
                ...
                . ' SUM(CASE WHEN disposition = 1 THEN rcount ELSE 0 END) AS quarantined FROM '
